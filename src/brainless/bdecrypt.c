// Cyborg BRAINLESS Decryption Protocol v20260430
...
// Brainless Decrypt: reverse XOR chain
void bdecrypt(uint8_t* ptrData_a, size_t intLen_a)
{
    if (intLen_a < 2) return;
    
    // Reverse Ouroboros Step first
    ptrData_a[intLen_a - 1] = ptrData_a[intLen_a - 1] ^ ptrData_a[0];
    
    // Forward XOR Chain Restoration
    for (size_t intI = 0; intI < intLen_a - 1; intI++) {
...
int main(int intArgC_a, char* strArgv_a[])
{
    ...
    printf("BRAINLESS Decryption Protocol v20260430\n");
        ...
        if (ptrInput) {
            ...
            if (ptrData) {
                ...
                if (ptrOutput) {
                    ...
                    fclose(ptrOutput);
                    blnDecryptOk = true;
                    printf("Decrypt successful: %s -> %s (%ld bytes)\n", strArgv_a[1], strArgv_a[2], intLen);
                } else {
