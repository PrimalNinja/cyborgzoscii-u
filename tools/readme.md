...
*   **Activate** — Decodes a stored ROM and saves it as the active ROM for use with the Encode and Decode tools in the Explorer menu.
*   **Deactivate** — Securely deletes the active ROM from disk.
...
When you activate a ROM from the ROMs explorer, the system decodes it and writes the result to a file called `active.rom` in your data folder. This file contains the fully decoded ROM image, which is indexed to build the address table used by the Explorer menu Encode and Decode tools.

**Important: `active.rom` is NOT secured while it is active.** By design, it must exist in decoded form on disk so that the Explorer menu Encode and Decode tools can use it. You should be aware that while `active.rom` exists on disk it is readable. Use the **Deactivate** function (or the Deactivate button in the Status screen) to securely delete `active.rom` when you are finished with it. The resident ROM you log in with is entirely separate — it is held only in memory and is never written to disk.
